...
export default function BoardSideHeader() {
    ...
    return <div className="w-full h-full dark:text-grayscale1">
        ...
        <div className="font-bold text-2xl mt-10 cursor-pointer">{subCategory.map((sub: string, index: number) =>
            <div key={index} className={index === selectedCategory ? "py-5 px-10 rounded-xl bg-primary1 dark:bg-primary4" : "py-5 px-10 rounded-xl"} onClick={() => setSelectedCategory(index)}>{sub}</div>)}
        </div>
        </div>
}